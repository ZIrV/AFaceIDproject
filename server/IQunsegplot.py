    ...
    for file in files:
        ...
        # if len(res) == 3 and int(res[0]) == 3 and int(res[1]) >= 30 and int(res[1]) < 31:
            ...
            for channelID in [1,6]:
                ...
                # plt.figure()
                letter_size = 35
                font = {'weight': 'bold', 'size': letter_size}
                # plt.pcolormesh(t, freq, zxx2)
                # plt.ylim((fc-50, fc+50))
                # plt.xlabel('Time (s)',font)
                # plt.ylabel('Frequency(Hz)',font)
                # plt.tick_params(labelsize=letter_size)
                # cb = plt.colorbar()
                # cb.ax.tick_params(labelsize=letter_size)
                ...
                delta_diffph = [0]*5
                for i in range(5, len(signal_diffph)):
                    delta_diffph.append((signal_diffph[i] - signal_diffph[i-5])*1)
                delta_diffam = [0]*5
                for i in range(5, len(signal_diffam)):
                    delta_diffam.append((signal_diffam[i] - signal_diffam[i-5])*1)
                delta_diffph = np.array(delta_diffph)
                delta_diffam = np.array(delta_diffam)
                t = [i/480 for i in range(0, len(delta_diffph))]
                # plt.figure()
                # plt.subplot(211)
                plt.plot(t, signal_diffph, label=channelID, linewidth=4)
                plt.xlabel('Time (s)', font)
                plt.ylabel('Phase', font)
                plt.xlim((0, 2.9))
                plt.tick_params(labelsize=letter_size)
                # plt.legend()
                # plt.subplot(212)
                # plt.plot(t, delta_diffam, label=channelID, linewidth=4)
                # plt.xlabel('Time(s)', font)
